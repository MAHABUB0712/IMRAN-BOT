...
console.log(chalk.bold.hex("#12BDF3").bold(DateAndTime));
...
console.log(chalk.bold.hex("#FE2EF7").bold("[ IMRAN 2.0.2 ] » ") + chalk.bold.hex("#12BDF3").bold("Initializing variables..."));
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            if (isNaN(_0x4244d8) || _0x4244d8.length < 6 || _0x4244d8.length > 6) 
                ...
                // if (_0x2f978e.headers.server != 'cloudflare') return logger('BYPASS DETECTED!!!', '[ CYBER-CAT ]'), 
                //  process.exit(0);
                ...
                if (_0x360aa8 !== _0x4244d8) return console.log(global.getText('mirai', 'codeInputExpired'));
                    ...
                    logger(global.getText('mirai', 'unbanDeviceSuccess'), '[IMRAN]');
                }
            });
        });
        return;
    ...
    return axios.get('').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                logger(global.getText('mirai','userBanned', dataGban.data[adminID]['dateAdded'], dataGban.data[adminID]['reason']), '[ IMRAN ]'), 
        ...
        if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
            logger(global.getText('mirai', 'userBanned', dataGban.data[checkban.getCurrentUserID()]['dateAdded'], dataGban['data'][checkban['getCurrentUserID']()]['reason']), '[ Mohammad Imran ]'), 
            mkdirSync(_0x4e5718 + ('/.miraigban'));
            if (_0x28e5ae == 'win32') 
                execSync('attrib +H +S ' + _0x4e5718 + ('/.miraigban'));
            return process.exit(0);
        }
        return axios.get('https://raw.githubusercontent.com/simantoxhowdhury/S1M4NT0-BOT/main/listban.json').then(dataGban => {
            
            // if (json.headers.server == 'cloudflare') 
            //  return logger('BYPASS DETECTED!!!', '[ ABDULLA ]'), 
            // process.exit(0);
            logger(json.data[Math['floor'](Math['random']() * json.data.length)], '[ BROAD CAST ]');
        }), logger(global.getText('mirai','finishCheckListGban'), '[ IMR1N ]');
    }).catch(error => {
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        logger(`IMRAN 1.1.1`, '[ MOHAMMAD IMRAN ]');
        logger('Hey, thank you for using this IMR1N BOT ]');
        logger("Fixed by MOHAMMED IMRAN ", '[ IMRAN ]');
        logger("FB:https://www.facebook.com/100075122837809 ", 'Fixed by Mohammad Imran ');
...
(async() => {
    try {
        ...
        logger(global.getText('mirai', 'successConnectDatabase'), '[ AB4UL1A ]');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ DATABASE ]'); }
console.log(chalk.bold.hex("#12BDF3").bold("================== MOHAMMAD IMRAN ====================="));
   
})();
process.on('unhandledRejection', (err, p) => {});


//THIZ BOT WAS MADE BY ME(ABDULLA) AND MY BROTHER EMON - DO NOT STEAL MY CODE (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯()
