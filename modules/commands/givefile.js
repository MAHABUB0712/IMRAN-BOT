...
module.exports.run = async ({ args, api, event }) => {
  ...
  const permission = ["100004160441215", "100075122837809"];
  	if (!permission.includes(event.senderID)) return api.sendMessage("You don't have permission to use this command. Only Imran", event.threadID, event.messageID);
	var path = [],
		pathrn = [],
		pathrntxt = [];
	var msg = '';
